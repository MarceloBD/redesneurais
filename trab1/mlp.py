"""
A library for multi layer perceptron for any number of hidden layers
...
class Mlp():
	...
	def __init__(self, input_neurons, output_neurons, hidden_neurons, hidden_layers):
		...
		number_weights_in_layer  = int(self.input_neurons*self.hidden_neurons)
		number_weights_hid_layer = int(self.hidden_neurons*self.hidden_neurons)
		number_weights_out_layer = int(self.hidden_neurons*self.output_neurons)

		self.input_weights = [random.uniform(0, 1) for _ in range(number_weights_in_layer)]
		for i in range(hidden_layers):
			self.hidden_weights[i] = [random.uniform(0, 1) for _ in range(number_weights_hid_layer)]
		self.output_weights = [random.uniform(0, 1) for _ in range(number_weights_out_layer)]
		
		for i in range(hidden_layers):
			self.bias_hid_weights[i] = [random.uniform(0, 1) for _ in range(self.hidden_neurons)]
		self.bias_out_weights = [random.uniform(0, 1) for _ in range(self.output_neurons)]

		number_total_neurons = self.input_neurons + self.output_neurons + hidden_layers * hidden_neurons
		self.output = [0 for _ in range(number_total_neurons)]
		
		for i in range(hidden_layers):
			self.hidden_derivatives[i] = [0 for _ in range(number_weights_hid_layer)]
		self.output_derivatives = [0 for _ in range(number_weights_out_layer)]
		self.output_delta = [0 for _ in range(self.output_neurons)]

		self.bias_out_derivatives = [0 for _ in range(self.output_neurons)]
		for i in range(hidden_layers):
			self.bias_hid_derivatives[i] = [0 for _ in range(self.hidden_neurons)]
	...
	def total_error(self):
		...
		"""
		error = 0x
		first_output_neuron = self.input_neurons + self.hidden_neurons*self.hidden_layers 
	...
	def update_all_neurons(self):
		...
		for i in range(self.hidden_layers):
			first_neuron_layer = self.input_neurons + i*self.hidden_neurons
			for j in range(self.hidden_neurons):
				self.output[first_neuron_layer + j] = self.processes_input(self.hidden_total_net_input(j)) 	# see this method

		first_output_neuron = self.input_neurons + self.hidden_neurons*self.hidden_layers
		for i in range (self.output_neurons):
			self.output[first_output_neuron + i] = self.processes_input(self.ouput_total_net_input(i))
	...
	def ouput_total_net_input(self, neuron_number):
		...
		for i in range(self.hidden_neurons):
			total_input += self.output_weights[i*self.output_neurons + neuron_number]*self.output[i + self.output_neurons]
		
		total_input += self.bias_out_weights[neuron_number]
		return total_input 
